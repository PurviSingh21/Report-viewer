...
import { listRuns, getAllFailedForRun } from "@/lib/reportReader";
...
export default async function FailedJsonPage({
  searchParams,
}: {
  ...
  const failed = await getAllFailedForRun(currentRun);
    ...
    <>
      ...
      {failed.length ? (
        <JsonViewer
          data={failed}
          title="failed.json"
          filename={`failed-${currentRun}.json`}
        />
      ) : (
        <EmptyState
          title="No failed data"
          message="No failed.json files found for this run — all tests may have passed!"
        />
      )}
    </>
  );
    ...
    <div className="mb-8 flex items-center justify-between">
      <div>
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-bold text-slate-900">Failed JSON</h1>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Raw failed.json data with syntax highlighting
        </p>
      </div>
      <Suspense>
